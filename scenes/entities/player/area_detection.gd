...
signal facingLedge(ledgePos: Vector3, wall_collision_normal: Vector3)
...
func ledge_detection() -> void:
	#var wallDist: Vector3 = wall_detection_ray.get_collision_point()
	...
	if ledge_finder_ray.is_colliding():
		facingLedge.emit(ledge_finder_ray.get_collision_point(), wall_detection_ray.get_collision_normal())
